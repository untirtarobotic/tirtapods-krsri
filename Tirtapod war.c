...
void loop () {
  ...
  if (activation::isON) {
    if (activation::isLowMove) {
      if(isHomeState == true && ((jarak_PingC >= 56) && (jarak_PingC <= 58))){
        lcd::justPrint("Langsung Gass", "Moving forward + ");
        legs::forward(true);
      }
      else if(isHomeState == true && (jarak_PingC > 58)){
        lcd::justPrint("Muter balik", "Moving forward + ");
        legs::rotateCW(3000);   // Anggap muter 180 derajat
      }
      else if(isHomeState == true && (jarak_PingC < 10)){
        legs::rotateCW(1500);   // Anggap muter 90 derajat
        if(isHomeState == true && ((jarak_PingC >= 56) && (jarak_PingC <= 58))){
          lcd::justPrint("Langsung Gass", "Moving forward + ");
          legs::forward(true);
        }
